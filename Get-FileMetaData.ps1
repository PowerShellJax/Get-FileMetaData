...
#Example
Get-ChildItem -Path E:\ -Recurse -Filter *.mov | Select-Object name, CreationTime, Directory | Sort-Object -Property CreationTime -OutVariable fileOrder | Out-Null
Write-Host "test `n`n"
$fileOrder | Select-Object -First 1 -OutVariable NewestFile | Out-Null
$fileOrder | Select-Object -Last 1 -OutVariable OldestFile | Out-Null
$NewestFile, $OldestFile

#Example #2
Get-Childitem -Path E:\Lock\F -recurse -Filter emer200114*.mov | Get-FileMetaData -OutVariable testvar | Select-Object name, length
Write-Host `n 
foreach($test in $testvar){
        $totaltime += [timespan]$test.length
    }
